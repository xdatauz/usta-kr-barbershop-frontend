		...
		<section className="rounded-3xl border border-slate-300/70 bg-white/80 p-4 shadow-sm backdrop-blur sm:p-6 lg:p-8 dark:border-slate-700 dark:bg-slate-900/70">
			<div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
				<div className="max-w-2xl space-y-2">
					<p className="text-xs font-semibold uppercase tracking-[0.17em] text-emerald-700 dark:text-emerald-300">{t("servicesSection.eyebrow")}</p>
					<h2 className="text-2xl font-black text-slate-900 sm:text-3xl dark:text-slate-50">{t("servicesSection.title")}</h2>
					<p className="text-sm leading-7 text-slate-700 sm:text-base dark:text-slate-300">{t("servicesSection.description")}</p>
				</div>
				<Link
					to={`/${locale}/contact`}
						...
						>
							<div className="relative h-48 overflow-hidden">
								...
								<div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/70 to-transparent p-3">
									...
									<p className="rounded-full bg-emerald-500 px-2.5 py-1 text-xs font-semibold text-slate-950">{t(`servicesSection.items.${service.key}.price`)}</p>
								</div>
							</div>
							<div className="space-y-2 p-4">
								<div className="flex items-center gap-2">
									<Icon className="h-4 w-4 text-emerald-600 dark:text-emerald-300" />
									<h3 className="text-base font-bold text-slate-900 dark:text-slate-50">{t(`servicesSection.items.${service.key}.name`)}</h3>
								</div>
								<p className="text-sm leading-6 text-slate-700 dark:text-slate-300">{t(`servicesSection.items.${service.key}.description`)}</p>
							</div>
						</motion.article>
					);
				})}
